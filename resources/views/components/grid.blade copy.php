    ...
    @if (!$visualizacao)
        ...
        <script>
            ...
            gridConfig['{{$id}}'] = {
                ...
                linha: '<div class="form-group"><div class="d-flex">{!! preg_replace(['/\s*\r?\n\s+/', '/\s+/'],['', ' '], $slot) !!}<button type="button" class="btn btn-danger btn-grid-remove" title="Remover"><i class="far fa-trash-alt"></i></button></div></div>'
            }
        </script>
    @else
